...
async function runPlaywrightChecks() {
  ...
  try {
    ...
    assertOk(!(await sonificationToggle.isDisabled()), "continuous speaker should stay enabled in piano mode");
    await sonificationToggle.click();
    assertOk((await sonificationToggle.getAttribute("aria-pressed")) === "true", "sonification should start while the piano is open");
    await sonificationToggle.click();
    assertOk((await sonificationToggle.getAttribute("aria-pressed")) === "false", "sonification should stop while the piano is open");
    ...
    assertOk(await page.getByRole("heading", { name: "Piston", exact: true }).isVisible(), "Piston heading was not visible");
    assertOk((await page.locator("#heatEngineCanvas").getAttribute("aria-label")) === "heat-engine piston visualization", "Piston canvas should retain its heat-engine context for accessibility");
    ...
    assertOk(await page.locator("#plotGrid .plot-panel canvas:visible").count() === 12, "expected twelve visible plot canvases");
    ...
    assertOk(plotIds.join("|") === "model|heatEngine|work|light|velocity|tpOpacity|periodogram|phasePortrait|time|lum|stability|strip", `plot grid should include all removable panels, saw ${plotIds.join("|")}`);
    ...
    assertOk(Boolean(heatPanel), "Piston panel was missing from the plot grid");
    assertOk(Boolean(workPanel), "Work panel was missing from the plot grid");
    assertOk(workPanel.top === heatPanel.top, "Work panel should sit on the Piston row");
    assertOk(workPanel.left > heatPanel.left, "Work panel should sit to the right of Piston");
    assertOk(firstRow.find((panel) => panel.id === "model")?.width < 360, "Shell should stay compact");
    assertOk(heatPanel.width < 360, "Piston should stay compact");
    ...
    assertOk(plotLayout.panels.find((panel) => panel.id === "tpOpacity")?.width > 360, "T-P Loop should expand to fill its flex row");
    assertOk(plotLayout.panels.find((panel) => panel.id === "periodogram")?.width > 360, "Periodogram should expand to fill its flex row");
    ...
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-tp-opacity-mode")) === "single", "T-P Loop should start in single-model mode");
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-axis-labels")) === "log10(T/T_0),log10(P/P_0)", "T-P Loop should expose temperature-pressure axes");
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-color-variable")) === "log10(kappa/kappa_0)", "T-P Loop should color by opacity in single mode");
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-opacity-colorbar")) === "log10(kappa/kappa_0)", "T-P Loop should expose opacity colorbar metadata in single mode");
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-opacity-contours")) === "log10(kappa/kappa_0)", "T-P Loop should expose opacity contours");
    ...
    assertOk((await page.locator("#periodogramCanvas").getAttribute("data-axis-labels")) === "frequency (τ⁻¹),power [(ΔL/L₀)²]", "Periodogram should display frequency and power units");
    assertOk((await page.locator("#periodogramCanvas").getAttribute("data-periodogram-frequency-unit")) === "tau^-1", "Periodogram should expose inverse-tau frequency units");
    assertOk((await page.locator("#periodogramCanvas").getAttribute("data-periodogram-power-unit")) === "(delta_L_over_L0)^2", "Periodogram should expose squared fractional-luminosity power units");
    assertOk((await page.locator("#periodogramCanvas").getAttribute("data-periodogram-grid")) === "none", "Periodogram should not draw grid lines");
    ...
    assertOk(tpOpacityHasPaint, "T-P Loop canvas was blank");
    ...
    assertOk((await page.locator("#plotGrid").getAttribute("data-visible-plots")) === "11", "eleven visible plots should be tracked");
    ...
    assertOk(await page.locator("#plotGrid .plot-panel canvas:visible").count() === 11, "expected eleven visible plot canvases after hiding Shell");
    ...
    assertOk((await page.locator("#plotGrid").getAttribute("data-visible-plots")) === "12", "twelve visible plots should be tracked after restore");
    ...
    assertOk(await page.locator("[data-plot-panel='heatEngine']").isHidden(), "Piston should be hidden in grid mode");
    ...
    assertOk(await page.locator("[data-plot-panel='periodogram']").isVisible(), "Periodogram should remain visible in grid mode");
    ...
    assertOk((await page.locator("#plotGrid").getAttribute("data-visible-plots")) === "8", "eight visible plots should be tracked in grid mode");
    ...
    assertOk(await page.locator("#hiddenPlotControls [data-plot-toggle='heatEngine']").isDisabled(), "Piston toggle should be disabled in grid mode");
    ...
    assertOk(!(await page.locator("[data-plot-toggle='periodogram']").isDisabled()), "Periodogram toggle should remain enabled in grid mode");
    ...
    assertOk(!(await sonificationToggle.isDisabled()), "speaker should stay enabled in grid piano mode");
    await sonificationToggle.click();
    assertOk((await sonificationToggle.getAttribute("aria-pressed")) === "true", "speaker should start in grid piano mode");
    await sonificationToggle.click();
    ...
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-color-variable")) === "grid parameter", "T-P Loop should color grid loops by the grid parameter");
    assertOk((await page.locator("#tpOpacityCanvas").getAttribute("data-opacity-contours")) === "log10(kappa/kappa_0)", "T-P Loop should keep opacity contours in grid mode");
